<div class="card">
    ...
    <div class="card-body">
        <x-input type="password" title="应用ID" name="alipay[app_id]" value="{{ config('site.alipay.app_id') }}"
            placeholder="请登录支付定应用中心查看  https://open.alipay.com/platform/developerIndex.htm">
        </x-input>

        <x-input title="编码格式" name="alipay[charset]" value="{{ config('site.alipay.charset','UTF-8') }}"
            placeholder="编码格式一般使用UTF-8不需要更改">
        </x-input>

        <x-input title="签名方式" name="alipay[sign_type]" value="{{ config('site.alipay.sign_type','RSA2') }}"
            placeholder="签名方式一般不需要更改">
        </x-input>

        {{-- <x-input title="支付宝网关" name="alipay[gatewayUrl]"
        ...
        <x-input type="password" title="商户私钥" name="alipay[private_key]" placeholder="使用支付宝提供的签名生成工具创建，注意不要有换行"
            value="{{ config('site.alipay.private_key') }}"></x-input>
